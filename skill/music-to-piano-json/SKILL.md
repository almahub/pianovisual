...
- Produce three logical roles but map them to two playable hands: melody/voice to the right hand; normally one bass note plus two characteristic chord tones to the left hand.
...
- Keep the left hand deliberately easy: normally three simultaneous notes, never wider than one octave, one harmonic attack per stabilized chord, and economical movement between chord tones. Permit a fourth note only for a confidently recognized seventh chord whose complete voicing has comfortable adjacent intervals; otherwise fall back automatically to three or fewer notes. Exclude the declared bass pitch class from upper notes when other chord tones are available.
